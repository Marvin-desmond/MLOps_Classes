...
@task 
def get_paths(date: datetime) -> tuple[str, str]:
    if not date:
        date = datetime.today()
    year = date.year
    month = date.month
    return (
    f"./data/fhv_tripdata_{year}-{month - 2:02d}.parquet",
    f"./data/fhv_tripdata_{year}-{month - 1:02d}.parquet",
    )
